...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("Layer", initializer=xavier_weight_init()):
            self.W = tf.get_variable("W", (self.config.n_features * self.config.embed_size, self.config.hidden_size), tf.float32)
            self.b1 = tf.get_variable("b1", (self.config.hidden_size,), tf.float32)

        with tf.variable_scope("Layer", initializer=xavier_weight_init()):
            self.U = tf.get_variable("U", (self.config.hidden_size, self.config.n_classes), tf.float32)
            self.b2 = tf.get_variable("b2", (self.config.n_classes,), tf.float32)

        hidden = tf.nn.tanh(tf.matmul(tf.to_float(x), tf.nn.dropout(self.W, self.dropout_placeholder)) + self.b1)
        pred = tf.matmul(hidden, tf.nn.dropout(self.U, self.dropout_placeholder)) + self.b2
        ### END YOUR CODE
        return pred
